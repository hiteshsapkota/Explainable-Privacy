...
public class MainController {
  ...
  public String paymentForm(@ModelAttribute Payment payment, Model model) {
    ...
    List<Record> records = jdbcTemplate.query(sql, new Object[] { payment.getUsername() },
        new RecordRowMapper());

    if (records.size() >= 30) {

      sql = "select * from payment where user_name=?";
      String code = jdbcTemplate
          .query(sql, new Object[] { payment.getUsername() }, new PaymentRowMapper()).get(0)
          .getCode();
      if (code.equals("NA")) {
        payment.generateCode(5);
        sql = "update payment set code= " + "'" + payment.getCode() + "'" + " where user_name= "
            + "'" + payment.getUsername() + "'";
  ...
  public String evaluationForm(Model model) {
    ...
    if (evaluation.getImage_id().equals("undefined") || evaluation.getImage_id().equals("na")) {
      ...
      try {
        ...
        String command = configurationService.getPythonCommand() + " " + python_root_dir
            + "utils.py generateImageID" + " " + evaluation.getUsername() + " " + "evaluation" + " "
            + eval_batch_size;
        ...
        command = configurationService.getPythonCommand() + " " + python_root_dir
            + "models.py generateExplanation" + " " + evaluation.getUsername();
    ...
    if ((Integer.parseInt(index.getEvalCompleted()) + 1)
        % configurationService.getUpdate_epoch() == 0)
    ...
    try {
      ...
      for (String imageAttribute : imageAttributes) {
        ...
        records = jdbcTemplate.query(command, new Object[] { imageAttribute },
            new AttributeRowMapper());
        Attribute record = records.get(0);
        feedback.setAttributeId(
            imageAttribute.replace(" ", "-").replace("(", "").replace(")", "").replace("/", ""));
